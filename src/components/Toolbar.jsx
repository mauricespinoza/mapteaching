...
export const TOOLS = [
  ...
  { id: 'split', label: 'Cortar línea', icon: CutLineIcon, key: 'K' },
    ...
    <div className="flex h-full w-[4.25rem] shrink-0 flex-col items-center gap-1 overflow-y-auto border-r border-slate-200 bg-white py-2 md:w-[5.25rem]">
      ...
      <div className="mt-2 w-[3.75rem] border-t border-slate-200 pt-2 text-center md:w-[4.75rem]">
        <p
          ...
          title={t('Un toque coloca un vértice; mantener y arrastrar dibuja un trazo continuo')}
        >
          {t('Toque = vértice')}<br />{t('Arrastrar = trazo')}
        </p>
        <button
          ...
          title={t('Con el lápiz activo, los dedos sólo navegan (rechazo de palma)')}
        >
          {penOnly ? t('Sólo lápiz') : t('Dedo dibuja')}
        </button>
      </div>
    </div>
  )
